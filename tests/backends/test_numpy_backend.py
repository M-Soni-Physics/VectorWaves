import pytest
import numpy as np
from vectorwaves.backends.numpy_backend import NumpyMethods

# =========================================================================
#                    1. POINT COMPUTATION TESTS
# =========================================================================

def test_numpy_compute_point_correctness(simple_test_beam):
    backend = NumpyMethods(simple_test_beam)
...
def test_numpy_compute_point_options(simple_test_beam):
    backend = NumpyMethods(simple_test_beam)
    x, y, z, t = 0.1, 0.2, 0.3, 0.4
...
# =========================================================================
#                    2. CLOUD COMPUTATION TESTS
# =========================================================================

def test_numpy_compute_cloud(simple_test_beam):
    backend = NumpyMethods(simple_test_beam)
...
def test_numpy_compute_cloud_batching_and_callback(simple_test_beam):
    backend = NumpyMethods(simple_test_beam, max_points_per_batch=2)
...
def test_numpy_compute_cloud_options(simple_test_beam):
    backend = NumpyMethods(simple_test_beam)
    x = np.array([0.1, 0.2])
    y = np.array([0.2, 0.3])
    z = np.array([0.3, 0.4])
...
# =========================================================================
#                    3. GRID COMPUTATION TESTS
# =========================================================================

def test_numpy_compute_grid(simple_test_beam):
    backend = NumpyMethods(simple_test_beam)
...
def test_numpy_compute_grid_batching_and_callback(simple_test_beam):
    backend = NumpyMethods(simple_test_beam, max_points_per_batch=4)
...
def test_numpy_compute_grid_options(simple_test_beam):
    backend = NumpyMethods(simple_test_beam)
...
# =========================================================================
#                    4. PHYSICAL MAXWELL CONSISTENCY
# =========================================================================

def test_numpy_maxwell_relations(simple_test_beam):
    backend = NumpyMethods(simple_test_beam)
    ...
    # 1. Divergence of E must be zero for transverse waves
    div_E = dx[0] + dy[1] + dz[2]
    assert np.isclose(div_E, 0.0, atol=1e-13)
    
    # 2. Curl of E = i * omega * B
    curl_E = np.array([
        dy[2] - dz[1],
        dz[0] - dx[2],
        dx[1] - dy[0]
